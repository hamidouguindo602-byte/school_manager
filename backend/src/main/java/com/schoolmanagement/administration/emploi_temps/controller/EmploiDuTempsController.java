...
public class EmploiDuTempsController {
  ...
  public ResponseEntity<Map<String, List<EmploiDuTempsResponse>>> getGrilleParClasse(
      @PathVariable Long id) {
    return ResponseEntity.ok(service.obtenirGrilleParClasse(id));
  }

  @PostMapping
  public ResponseEntity<EmploiDuTempsResponse> create(
      @Valid @RequestBody EmploiDuTempsRequest req) {
    return ResponseEntity.ok(service.creerCreneau(req));
  }

  @PutMapping("/{id}")
  public ResponseEntity<EmploiDuTempsResponse> update(
      @PathVariable Long id, @Valid @RequestBody EmploiDuTempsRequest req) {
