...
public class AbsenceService {
  ...
  public List<AbsenceResponse> obtenirAbsencesParClasse(Long classeId) {
    return absenceRepository.findByClasseId(classeId).stream()
        .map(this::mapToDto)
        .collect(Collectors.toList());
  }

  @Transactional(readOnly = true)
  public List<AbsenceResponse> obtenirAbsencesParClasseEtPeriode(
      Long classeId, LocalDate debut, LocalDate fin) {
    return absenceRepository.findByClasseIdAndDateBetween(classeId, debut, fin).stream()
        .map(this::mapToDto)
        .collect(Collectors.toList());
  }
  ...
  public List<AbsenceResponse> obtenirAbsencesParEnseignant(Long enseignantId) {
    return absenceRepository.findByEnseignantId(enseignantId).stream()
        .map(this::mapToDto)
        .collect(Collectors.toList());
  }
  ...
  public AbsenceStatsResponse getStatsParClasse(Long classeId) {
    ...
    long justifiees =
        absences.stream().filter(absence -> Boolean.TRUE.equals(absence.getJustifiee())).count();
  ...
  @Transactional
  public AbsenceResponse justifierAbsence(
      Long id, JustificationRequest req, MultipartFile fichierPdf) {
    Absence abs =
        absenceRepository
            .findById(id)
