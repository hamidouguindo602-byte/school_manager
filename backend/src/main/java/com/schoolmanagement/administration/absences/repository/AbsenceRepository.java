...
public interface AbsenceRepository extends JpaRepository<Absence, Long> {
  ...
  @Query(
      "SELECT a FROM Absence a JOIN a.emploiDuTemps e JOIN e.assignation asg WHERE asg.classe.id ="
          + " :classeId")
  List<Absence> findByClasseId(@Param("classeId") Long classeId);

  @Query(
      "SELECT a FROM Absence a JOIN a.emploiDuTemps e JOIN e.assignation asg WHERE"
          + " asg.enseignant.id = :enseignantId")
  List<Absence> findByEnseignantId(@Param("enseignantId") Long enseignantId);

  @Query(
      "SELECT a FROM Absence a JOIN a.emploiDuTemps e JOIN e.assignation asg WHERE asg.classe.id ="
          + " :classeId AND a.dateAbsence BETWEEN :debut AND :fin")
