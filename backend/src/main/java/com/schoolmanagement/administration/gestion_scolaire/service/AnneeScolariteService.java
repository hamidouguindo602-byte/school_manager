...
public class AnneeScolariteService {
    ...
    public AnneeScolariteResponse creerAnneeScolarite(AnneeScolariteCreationRequest request) {
        AnneeScolarite anneeScolarite = AnneeScolarite.builder()
    ...
    public AnneeScolariteResponse obtenirAnneeScolariteParId(Long id) {
        AnneeScolarite annee = anneeScolariteRepository.findById(id)
                .orElseThrow(() -> new EntityNotFoundException("Année scolaire introuvable avec l'ID : " + id));
    ...
    @Transactional
    public AnneeScolariteResponse mettreAJourAnneeScolarite(Long id, AnneeScolariteUpdateRequest request) {
        AnneeScolarite annee = anneeScolariteRepository.findById(id)
                .orElseThrow(() -> new EntityNotFoundException("Année scolaire introuvable avec l'ID : " + id));
    ...
    public AnneeScolariteResponse activerAnneeScolarite(Long id) {
        ...
        // Optionnel : Vous pouvez aussi désactiver les autres années ici si une seule doit être active à la fois
