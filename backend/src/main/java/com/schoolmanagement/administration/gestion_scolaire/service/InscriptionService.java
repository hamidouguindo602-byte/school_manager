...
public class InscriptionService {
    ...
    public InscriptionResponse creerInscription(InscriptionRequest request) {
        Eleve eleve = eleveRepository.findById(request.getIdEleve())
                .orElseThrow(() -> new EntityNotFoundException("Élève non trouvé avec l'ID : " + request.getIdEleve()));

        Formation formation = formationRepository.findById(request.getIdFormation())
                .orElseThrow(() -> new EntityNotFoundException("Formation non trouvée avec l'ID : " + request.getIdFormation()));

        Classe classe = classeRepository.findById(request.getIdClasse())
                .orElseThrow(() -> new EntityNotFoundException("Classe non trouvée avec l'ID : " + request.getIdClasse()));
        ...
        Inscription inscription = Inscription.builder()
                ...
                .statut(request.getStatut() != null ? request.getStatut().name() : StatusInscription.en_attente.name())
                .build();
    ...
    public InscriptionResponse validerInscription(Long id) {
        Inscription inscription = inscriptionRepository.findById(id)
                .orElseThrow(() -> new EntityNotFoundException("Inscription non trouvée avec l'ID : " + id));
    ...
    private InscriptionResponse mapToResponse(Inscription inscription) {
        return InscriptionResponse.builder()
                ...
                .statut(inscription.getStatut() != null ? StatusInscription.valueOf(inscription.getStatut()) : null)
                .build();
    }
}