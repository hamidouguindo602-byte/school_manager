...
public class AnneeScolariteController {
    ...
    @PostMapping
    public ResponseEntity<AnneeScolariteResponse> creerAnneeScolarite(@Valid @RequestBody AnneeScolariteCreationRequest request) {
        return new ResponseEntity<>(anneeScolariteService.creerAnneeScolarite(request), HttpStatus.CREATED);
    }
    ...
    @PutMapping("/{id}")
    public ResponseEntity<AnneeScolariteResponse> mettreAJourAnneeScolarite(@PathVariable Long id, @Valid @RequestBody AnneeScolariteUpdateRequest request) {
