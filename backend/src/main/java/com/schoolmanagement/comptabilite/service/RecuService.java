...
public class RecuService {
    ...
    @Transactional(readOnly = true)
    public RecuResponse obtenirParPaiement(
            Long idPaiement
    ) {
        ...
        return recuRepository
                ...
                .orElseThrow(() ->
                        new RuntimeException(
                                "Reçu introuvable pour ce paiement"
                        )
                );
    }

    public RecuResponse creerPourPaiement(
            Paiement paiement
    ) {
        ...
        if (paiement.getStatut()
            ...
            throw new RuntimeException(
                    "Un reçu ne peut être créé "
                            + "que pour un paiement valide"
            );
        }

        if (recuRepository
                .existsByPaiementIdPaiement(
                        paiement.getIdPaiement()
                )) {

            return obtenirParPaiement(
                    paiement.getIdPaiement()
            );
        }
    ...
    private Recu trouverRecu(Long id) {
        ...
        return recuRepository.findById(id)
                .orElseThrow(() ->
                        new RuntimeException(
                                "Reçu introuvable avec l'id : "
                                        + id
                        )
                );
    }

    private RecuResponse toResponse(
            Recu recu
    ) {
    ...
    public byte[] genererPdf(Long idRecu) {
        ...
        try {
            ...
            document.add(
                    new Paragraph("REÇU DE PAIEMENT")
            );
