...
public class PaiementService {
    ...
    public PaiementResponse creer(PaiementRequest request) {
        ...
        Echeance echeance =
                echeanceRepository.findById(request.idEcheance())
                        .orElseThrow(() ->
    ...
    public PaiementResponse modifier(
            Long id,
        ...
        if (!ancienneEcheance
                .getIdEcheance()
                .equals(nouvelleEcheance.getIdEcheance())) {

            mettreAJourStatutEcheance(
                    nouvelleEcheance
            );
        }
    ...
    public void supprimer(Long id) {
        ...
        if (paiement.getStatut() == StatutPaiement.VALIDE) {
            throw new RuntimeException(
                    "Un paiement validé doit être annulé avant suppression"
            );
        }
    ...
    @Transactional(readOnly = true)
    public List<PaiementResponse> obtenirParEcheance(
            Long idEcheance
    ) {
        ...
        return paiementRepository
                .findByEcheanceIdEcheance(idEcheance)
                .stream()
                .map(this::toResponse)
                .toList();
    }

    private void mettreAJourStatutEcheance(
            Echeance echeance
    ) {

        BigDecimal totalPaye =
                paiementRepository
                        .findByEcheanceIdEcheance(
    ...
    private Paiement trouverPaiement(Long id) {
        ...
        return paiementRepository.findById(id)
                .orElseThrow(() ->
                        new RuntimeException(
                                "Paiement introuvable avec l'id : "
                                        + id
                        )
                );
    }

    private PaiementResponse toResponse(
            Paiement paiement
    ) {
