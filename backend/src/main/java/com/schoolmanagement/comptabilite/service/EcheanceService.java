...
public class EcheanceService {
    ...
    public EcheanceResponse obtenirEcheance(Long id) {
        ...
        Echeance echeance = echeanceRepository.findById(id)
                .orElseThrow(() ->
                        new RuntimeException("Échéance introuvable avec l'id : " + id)
                );
    ...
    public EcheanceResponse creerEcheance(EcheanceRequest request) {
        ...
        Echeance echeanceEnregistree =
                echeanceRepository.save(echeance);

        return convertirEnResponse(echeanceEnregistree);
    }

    public EcheanceResponse modifierEcheance(
            Long id,
            EcheanceRequest request
    ) {

        Echeance echeance = echeanceRepository.findById(id)
                .orElseThrow(() ->
                        new RuntimeException(
                                "Échéance introuvable avec l'id : " + id
                        )
                );
    ...
    @Transactional(readOnly = true)
    public List<EcheanceResponse> obtenirEcheancesParInscription(
            Long idInscription
    ) {
        ...
        return echeanceRepository
                .findByInscriptionId(idInscription)                .stream()
                .map(this::convertirEnResponse)
                .toList();
    }
    ...
    public List<EcheanceResponse> obtenirEcheancesEnRetard() {
        ...
        return echeanceRepository
                .findByDateLimiteBeforeAndStatutNot(
                        LocalDate.now(),
