...
@PreAuthorize("hasRole('ADMIN') or hasAuthority('ECHEANCE_GERER')")
public class EcheanceController {
  ...
  public ResponseEntity<List<EcheanceResponse>> obtenirToutesLesEcheances() {
    ...
    return ResponseEntity.ok(echeanceService.obtenirToutesLesEcheances());
  }

  @GetMapping("/{id}")
  public ResponseEntity<EcheanceResponse> obtenirEcheance(@PathVariable Long id) {

    return ResponseEntity.ok(echeanceService.obtenirEcheance(id));
  }
  ...
  public ResponseEntity<EcheanceResponse> creerEcheance(
    ...
    return ResponseEntity.status(HttpStatus.CREATED).body(echeanceService.creerEcheance(request));
  }

  @PutMapping("/{id}")
  public ResponseEntity<EcheanceResponse> modifierEcheance(
      @PathVariable Long id, @Valid @RequestBody EcheanceRequest request) {

    return ResponseEntity.ok(echeanceService.modifierEcheance(id, request));
  }
  ...
  public ResponseEntity<List<EcheanceResponse>> obtenirEcheancesEnRetard() {
    ...
    return ResponseEntity.ok(echeanceService.obtenirEcheancesEnRetard());
  }
}
